package deng.fzip
{
   ...
   public class FZip extends EventDispatcher
   {
         ...
         if(param1.bytesAvailable >= 4)
         {
            ...
            switch(_loc2_)
            {
               case 67324752:
                  ...
                  var_458 = new FZipFile(charEncoding);
                  break;
               case 33639248:
               case 101010256:
                  parseFunc = parseIdle;
      ...
      protected function progressHandler(param1:Event) : void
      {
         ...
         {
            close();
            if(!hasEventListener(FZipErrorEvent.const_701))
            {
               throw e;
            }
            dispatchEvent(new FZipErrorEvent(FZipErrorEvent.const_701,e.message));
         }
      }
      ...
      public function loadBytes(param1:ByteArray) : void
      {
         if(!var_98 && parseFunc == parseIdle)
         {
            ...
            {
               dispatchEvent(new FZipErrorEvent(FZipErrorEvent.const_701,"EOF"));
            }
         }
      }
      ...
      {
         if(var_458.parse(param1))
         {
            var_53.push(var_458);
            if(false)
            {
               var_135["null"] = var_458;
            }
            dispatchEvent(new FZipEvent(FZipEvent.const_1161,var_458));
            var_458 = null;
