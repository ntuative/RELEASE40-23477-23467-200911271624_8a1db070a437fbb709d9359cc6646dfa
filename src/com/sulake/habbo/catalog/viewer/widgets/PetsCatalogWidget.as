package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget
   {
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(var_401 >= var_297.length)
         {
            return "";
         }
         var petColor:IPetColor = var_297[var_401] as IPetColor;
         var color:uint = petColor.rgb;
         var p:String = _name + String.fromCharCode(10) + addZeroPadding(String(var_1460),3) + String.fromCharCode(10) + addZeroPadding(color.toString(16).toUpperCase(),6);
         return p;
      }
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         if(var_401 < 0 || var_401 > var_297.length)
         {
            var_401 = 0;
         }
         updateImage();
      ...
      private function onApproveNameResult(param1:CatalogWidgetApproveNameResultEvent) : void
      {
         ...
         (page.viewer.catalog as HabboCatalog).showPurchaseConfirmation(var_935.getWithIndex(0),page,p);
      }
      ...
      override public function init() : void
      {
         ...
         _loc2_.addEventListener(WindowEvent.const_253,onTextWindowEvent);
         var_935 = new Map();
         ...
         var_815 = getPetTypeIndexFromProduct(_loc5_.localizationId);
         var_988 = 0;
         var_297 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc11_ = _loc10_.avatarRenderManager.petDataManager.getPetData(var_815);
               if(_loc11_)
               {
                  var_988 = _loc11_.breeds.length;
                  var_297 = _loc11_.colors;
                  if(var_297 == null)
                  {
                     var_297 = [];
                  }
         ...
         var _loc6_:int = 0;
         while(_loc6_ < var_988)
         {
            _loc3_.push(_loc4_.getKey(getRaceLocalizationKey(var_815,_loc6_),getRaceLocalizationKey(var_815,_loc6_)));
            _loc6_++;
         }
         var_935.add(var_815,_loc5_);
         ...
         events.addEventListener(WidgetEvent.const_855,onApproveNameResult);
         _loc5_ = var_935.getWithIndex(0);
         if(_loc5_ != null)
         {
            events.dispatchEvent(new SelectProductEvent(_loc5_));
         }
         var _loc8_:* = [];
         for each(_loc9_ in var_297)
         {
      ...
      {
         var _loc6_:* = null;
         var _loc1_:Offer = var_935.getWithIndex(0);
         if(_loc1_ == null)
         {
            return;
         }
         ...
         if(var_401 >= 0 && var_401 < var_297.length)
         {
            _loc6_ = var_297[var_401] as IPetColor;
            if(_loc6_ != null)
            {
               _loc2_ = uint(_loc6_.rgb);
            }
         }
         var _loc3_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImageNew(var_815,var_1460,_loc2_,AvatarScaleType.const_50);
         if(_loc3_ == null)
         {
            return;
         }
         _loc3_.initActionAppends();
         _loc3_.appendAction(AvatarAction.const_297,AvatarAction.const_576);
